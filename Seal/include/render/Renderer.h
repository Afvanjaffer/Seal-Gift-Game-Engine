...
namespace Seal { namespace Component { namespace Render { 
    class MeshRenderer; 
} } };

namespace Seal { namespace Render {
	class Renderer {
		friend class Seal::Component::Render::MeshRenderer;
	public:
		...
		std::vector<Seal::Shader::ShaderProgram>& getPostShaders();
		...
		virtual void render(Seal::Scene::GameScene &scene);
		...
		void setSkybox(const std::shared_ptr<Seal::Graphics::Cubemap> 
				&sky);

		/// Sets the solid color which is to be used when rendering the
		/// background color of the scene.
		/// const Vector4f &col : The solid color.
		void setSolidColor(const Seal::Math::Vector4f &col);
	protected:
		static Renderer *renderer; // Singleton Instance

		// Antialiasing Variables
		AntiAliasing antiAliasing;
		Seal::Shader::ShaderProgram fxaaShader;

		// Color Space Variables
		ColorSpace colorSpace;
		Seal::Shader::ShaderProgram gammaShader;

		// Properties for a Background Color & Skybox
		BackgroundMode backgroundMode;
		std::shared_ptr<Seal::Geometry::Mesh> cubemapMesh;
		Seal::Math::Vector4f solidColor;
		Seal::Shader::ShaderProgram solidColorShader;
		std::shared_ptr<Seal::Graphics::Cubemap> skybox;
		Seal::Shader::ShaderProgram skyboxShader;
		...
		std::unique_ptr<Seal::Graphics::Texture2D> cShadowMapTexture;
		Seal::Shader::ShaderProgram cShadowMapShader;
		Seal::Shader::ShaderProgram cShadowMapLinearShader;
		// Variance Shadow Map Buffer, Texture and Shader
		int vShadowMapBuffer;
		std::unique_ptr<Seal::Graphics::Texture2D> vShadowMapTexture;
		std::unique_ptr<Seal::Graphics::Texture2D> vShadowMapTextureAA;
		Seal::Shader::ShaderProgram vShadowMapShader;
		Seal::Shader::ShaderProgram vShadowMapLinearShader;
		Seal::Shader::ShaderProgram vsmGaussianBlurShader;
		
		// Shaders for post processing the Final Image
		std::vector<Seal::Shader::ShaderProgram> postShaders;
		bool doPostProcess;
