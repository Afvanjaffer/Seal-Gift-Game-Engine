...
namespace Seal { namespace Render { namespace Deferred {
	class DeferredRenderer : public Seal::Render::Renderer {
	public:
		...
		void render(Seal::Scene::GameScene &scene);
	...
	private:
		...
		std::unique_ptr<Seal::Geometry::Mesh> quad;
		...
		Seal::Shader::ShaderProgram geometryShader;
		Seal::Shader::ShaderProgram quadShader;
		Seal::Shader::ShaderProgram stencilShader;
		
		// Shaders used by all of the lights
		Seal::Shader::ShaderProgram ambientShader;
		Seal::Shader::ShaderProgram pointLightShader;
		Seal::Shader::ShaderProgram directionalLightShader;
		Seal::Shader::ShaderProgram spotLightShader;

		// Meshes of the Light Volumes
		std::shared_ptr<Seal::Geometry::Mesh> lightVolumePoint;
		std::shared_ptr<Seal::Geometry::Mesh> lightVolumeSpot;
		...
		void renderLightAmbient(const Seal::Component::Light::AmbientLight
			&aL);
		...
		void renderLightDirectional(const Seal::Component::Light::
				DirectionalLight &dL, Seal::Scene::GameScene &scene);

		/// Renders the specified Point Light using Deferred Rendering.
		/// const PointLight &pL : The point light to be rendered.
		void renderLightPoint(const Seal::Component::Light::PointLight
			&pL);

		/// Renders the specified Spot Light using Deferred Rendering.
		/// const SpotLight &pL : The spot light to be rendered.
		/// const GameScene &scene : The scene to be rendered.
		void renderLightSpot(const Seal::Component::Light::SpotLight &sL,
				Seal::Scene::GameScene &scene);
		...
		void renderLightQuad(const Seal::Component::Light::BaseLight &bL,
			Seal::Shader::ShaderProgram &shader, const std::string
			&name);
		...
		///								  rendering the light quad.
		/// const string &name : The name of the light uniform in the Shader.
		void renderLightVolume(const Seal::Component::Light::BaseLight
			&bL, Seal::Geometry::Mesh &volume,
			Seal::Shader::ShaderProgram &shader, const std::string
			&name);
		...
		void renderPassGeometry(Seal::Scene::GameScene &scene);
		...
		void renderPassLight(Seal::Scene::GameScene &scene);
		...
		Seal::Graphics::Texture2D& renderPassPostProcess();
		...
		void renderPostProcessShader(Seal::Shader::ShaderProgram &shader,
				int &read, int &write);
		...
		///					   processed image is to be drawn into.
		void renderPostProcessShader(Seal::Shader::ShaderProgram &shader,
			const Seal::Graphics::Texture2D &read, const int &write);
		...
		void renderTextureShadowMap(
				const bool &linear,
				const Seal::Component::Light::Shadow &shadow,
				Seal::Scene::GameScene &scene,
				const Seal::Math::Vector3f &pos = 
					Seal::Math::Vector3f(),
		...
		///						   full screen quad.
		void renderTexture(const Seal::Graphics::Texture2D &tex);
		...
		///				   light.
		void stencilLightVolume(Seal::Geometry::Mesh &volume);
		...
		void writePointLightTransform(const Seal::Component::Light::
				PointLight &pL);
		...
		///						  to be transformed.
		void writeSpotLightTransform(const Seal::Component::Light::
				SpotLight &sL);
		...
		void writeShadowMapToShader(
				const Seal::Component::Light::ShadowType &shadow, 
				Seal::Shader::ShaderProgram &shader);
	};
} } }
