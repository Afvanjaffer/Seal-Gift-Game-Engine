...
namespace Seal { namespace Geometry {
	/// <summary>
	...
	public:
		...
		/// </returns>
		std::unique_ptr<Seal::Object::GameObject> getGameObjectClone() 
				const;
	...
	private:
		...
		std::unique_ptr<Seal::Object::GameObject> gameObject;

		/// <summary>
		/// Fetches the specified float material property from the given
		/// ASSIMP material and writes it to the specified Seal material.
		/// 
		...
		/// </param>
		/// <param name="mat">
		/// The Seal material, passed by reference, to which the material
		...
		bool fetchMaterialProperty(
				const aiMaterial &aMat, Seal::Graphics::Material &mat,
		...
		bool fetchMaterialProperty(
				const aiMaterial &aMat, Seal::Graphics::Material &mat,
				const std::string &prop, const Seal::Math::Vector3f &def,
				const char *pKey, unsigned int type, unsigned int idx);

		/// <summary>
		/// Fetches the specified material texture from the given ASSIMP
		/// material and writes it to the specified Seal material.
		/// </summary>
		/// <param name="aMat">
		/// The ASSIMP material from which the property should be extracted
		/// from.
		...
		bool fetchMaterialTexture(
				...
				const Seal::Graphics::Texture2DCommonFillColor &def,
				const int &type);
		...
		/// <summary>
		/// Creates a Seal Mesh from the specified ASSIMP Mesh. The mesh
		...
		/// </param>
		/// <returns>
		/// The shared pointer to the Seal Mesh.
		/// </returns>
		std::shared_ptr<Seal::Geometry::Mesh> processAiMeshGeometry(
				aiMesh *aMesh);

		/// <summary>
		/// Creates a Seal Material from the specified ASSIMP material.
		/// The Seal Material will be initialized using the Standard
		/// Seal Material and will have all of its properties initialized
		...
		/// </param>
		/// <returns>
		/// The shared pointer to the Seal Material.
		/// </returns>
		std::shared_ptr<Seal::Graphics::Material> processAiMeshMaterial(
			aiMaterial *aMat);

		/// <summary>
		/// Creates a Seal Game Object from the specified ASSIMP node. 
		...
		/// </param>
		/// <returns>
		/// The unique pointer to the Seal Game Object.
		/// </returns>
		std::unique_ptr<Seal::Object::GameObject> processAiNode(
				aiNode *aNode);
	};
