...
using Seal::Component::Physics::Space;
using Seal::Component::Render::MeshRenderer;
using Seal::File::ImageIO;
using Seal::File::ImageIOLoadException;
using Seal::Debug::Logger;
using Seal::Graphics::Material;
using Seal::Graphics::Texture2D;
using Seal::Graphics::Texture2DCommonFillColor;
using Seal::Object::GameObject;
using Seal::Math::Vector2f;
using Seal::Math::Vector3f;
using Seal::Math::Vector4f;
using Seal::Math::Quaternion;
...
namespace Seal { namespace Geometry {
	...
	std::shared_ptr<Mesh> Model::processAiMeshGeometry(aiMesh *aMesh) {
		...
		// Create a new Seal Mesh with the fetched vertex and index data.
	...
	std::unique_ptr<GameObject> Model::processAiNode(aiNode *aNode) {
		...
		for (unsigned int i = 0; i < aNode->mNumMeshes; i++) {
			// Convert the ASSIMP Mesh Geometry into Seal Mesh Geometry
			aiMesh *aMesh = this->scene->mMeshes[aNode->mMeshes[i]];
			std::shared_ptr<Mesh> mesh = this->processAiMeshGeometry(aMesh);
			
			// Convert the ASSIMP Mesh Material into Seal Mesh Material
			aiMaterial *aMat;
